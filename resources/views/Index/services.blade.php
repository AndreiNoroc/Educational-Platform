<!-- //* ==================== Services Section ==================== -->
<section id="services" class="services">
    <div class="container">

        <!-- //? Titlul sectiunii -->
        <div class="section-title">
            <h2>Services</h2>
            <h3>Our site offers the <span>services</span> below</h3>
        </div>

        <!-- //? Cele 4 carduri de oferte de nerefuzat -->
        <div class="row">
            <div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0">
                <a href="stocks">
                    <div class="icon-box">
                        <div class="icon"><i class="bx bx-line-chart"></i></div>
                        <h4 class="title">Stocks</h4>
                        <p class="description">Visualize real time updated stocks and learn how to read and understand them</p>
                    </div>
                </a>
            </div>

            <div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0">
                <a href="forum">
                    <div class="icon-box">
                        <div class="icon"><i class="bx bx-world"></i></div>
                        <h4 class="title">Forum</h4>
                        <p class="description">Discussion thread for any financial matter and curiosity about today's economy</p>
                    </div>
                </a>
            </div>

            <div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0">
                <a href="games">
                    <div  class="icon-box">
                        <div class="icon"><i class="bx bx-game"></i></div>
                        <h4 class="title">Games</h4>
                        <p class="description">In this section you can try several games that give others insides to your financial issues.</p>
                    </div>
                </a>
            </div>

            <div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0">
                <a href="recommendations">
                    <div class="icon-box">
                        <div class="icon"><i class="bi bi-clipboard-data"></i></div>
                        <h4 class="title">Recommendations</h4>
                        <p class="description">A section full of interesting stories and recommendations from actual entrepreneurs and businessmen.</p>
                    </div>
                </a>
            </div>

        </div>

    </div>
</section>
<!-- //* ==================== End Services Section ==================== -->
